...
class Player extends Component {
	...
	getAltText() {
		if (this.props.showRole) {
			return (
				this.getNameWithYouTag() + this.capitalizeFirstOnly(this.props.role)
			);
		} else {
	...
	render() {
		...
		if (this.props.showRole) {
			identity_components = (
				<>
					...
					<p
						id="player-identity-label"
						className={
							this.getRoleClass() + this.getClassName() + " force-update"
						}
					>
		...
		return (
			<div
				id="player-container"
				className={
					this.getHighlight() + this.getClassName() + this.getButtonClass()
				}
			...
			>
				...
				<p id="player-disabled-label" hidden={!this.props.disabled}>
